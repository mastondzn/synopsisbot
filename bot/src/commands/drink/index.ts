...
function hydratedRecently(commandUser: CommandUser): commandUser is CommandUser & { hydratedAt: Date; } {
    ...
    subcommands: [{
        ...
        run: async ({ message }) => {
            const user = await db.query.commandUsers.findFirst({
                where: (commandUsers, { eq }) => eq(commandUsers.twitchId, message.senderUserID),
            });
