...
export type ZFetchOptions<TSchema extends z.ZodType<unknown> = z.ZodType<unknown>> = RequestInit & {
    url: string | URL;
...
export function zfetch<TSchema extends z.ZodType<unknown> = z.ZodType<unknown>>(
    ...
    const promise = performFetch(options);
...
async function performFetch({ schema, url, throwHttpErrors, json, ...rest }: ZFetchOptions) {
    if (json) {
    ...
    return await fetch(url, rest).then((response) => {
