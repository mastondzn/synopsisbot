...
export const getAuthedUserById = async (db: Database, id: string): Promise<AuthedUser | null> => {
    const [user] = await db.select().from(authedUsers).where(eq(authedUsers.twitchId, id)).limit(1);
    return user ?? null;
};

export const getAuthedUserByIdThrows = async (db: Database, id: string): Promise<AuthedUser> => {
...
export const getChannelModeByLogin = async (db: Database, login: string) => {
